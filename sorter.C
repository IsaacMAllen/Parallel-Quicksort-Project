...
int32_t main(int32_t argc, char * argv[])
{
    ...
    runSort(QUICKSORT, "QuickSort failed.", data, size, threadCt);
	...
	int32_t * data, int32_t size, int32_t threadCt)
{
    ...
    std::function<Sorts *()> makeSeqSort[QUICKSORT] = {makeSeqQuickSort};
...
{
    if (argc < 2) usage();  //must include: sort -n <n> and at least one of -m, -q
    int32_t opt;
...
void usage()
{
    printf("usage: sort  -n <n> -t <t>\n\n");
    printf("\tRandomly generates an array of size (1 << <n>) integers and sorts the\n");
    printf("\tarray implementing quicksort using three different techniques.\n");
    printf("\tParallel speedups are provided using the results from the corresponding\n");
    printf("\tsequential sort. GPU speedups are provided uisng the results from the corresponding parallel sort.\n\n");
